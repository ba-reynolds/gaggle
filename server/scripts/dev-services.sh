...
port_in_use() {
  # Returns 0 (true) if something is listening on the given port.
  (ss -ltn 2>/dev/null || netstat -ltn 2>/dev/null) | grep -qE "[:.]${1}[[:space:]]"
}
...
start() {
  ...
  if port_in_use "${DB_PORT}"; then
    echo ">> Postgres already listening on port ${DB_PORT} (leaving it alone)"
  ...
  # Ensure the application database exists (Postgres creates only 'postgres').
  ...
  if port_in_use "6379"; then
    echo ">> Redis already listening on port 6379 (leaving it alone)"
