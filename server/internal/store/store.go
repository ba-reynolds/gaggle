...
type Store struct {
	...
	Posts interface {
		...
		Search(ctx context.Context, query string, filters models.PostSearchFilters, limit int, cursor string) (*models.PostFeed, error)
	...
	News interface {
		Create(ctx context.Context, tx *sql.Tx, news models.NewsLink) error
		GetForPosts(ctx context.Context, postIDs []int) (map[int]*models.NewsLink, error)
	}
	Badges interface {
...
func NewStore(db *sql.DB, logger *slog.Logger, mediaDir string) *Store {
	return &Store{
		...
		News:              &newsStore{db: db, logger: logger},
