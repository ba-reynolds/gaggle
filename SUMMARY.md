...
- **Tab + Enter now opens the post.** The card already had `tabIndex={0} role="link"` but no `onKeyDown`, so Tab would highlight the card (focus ring) and nothing happened on `Enter`. Added `handleKeyDown` (`Enter` only, per `role="link"` spec — `Space` still scrolls) at `web/src/components/FeedPost.tsx:300` and wired `onKeyDown={handleKeyDown}` on the Card. Guard `event.target !== event.currentTarget` ensures `Enter` on a focused inner control (Like/Reply/Bookmark/Share, hashtag/mention link) keeps its own action and does not also navigate the card. Plain `Enter` navigates same-tab; `Ctrl/Cmd+Enter` deliberately *not* added (per your “no” — keyboard new-tab stays via browser menu). Added `focus-visible:ring` so keyboard focus is visible on all themes (comic/kanagawa/etc.).
- **Clicking @username no longer pushes post then profile (back-button double hop).** The feed header’s avatar/name is wrapped in `UserHoverCard`, whose trigger was `<a href="/profile/…">` without `stopPropagation` (`web/src/components/UserHoverCard.tsx:49`). A click therefore fired **both** the inner profile link *and* the outer Card’s `navigate("/post/:id")`, pushing `feed → post → profile` onto history. Back then went `profile → post → feed` as reported. Fix: `UserHoverCard` trigger is now `<Link to={profileUrl} onClick/auxClick/mouseDown stopPropagation>` — inner link wins, outer card ignores. `FeedPost` also got defense-in-depth guards `if (target.closest("a, button")) return` in `handlePostClick`/`handleAuxClick` so any future inner anchor that forgets to stop still won’t double-push.
- **Inner controls no longer bubble middle-clicks to the card.** Every interactive child that already did `stopPropagation()` on `click` now also does it on `auxclick`/`mousedown` for button 1: the card's overflow menu, Reply/Repost/Like/Bookmark/Share actions, the quoted-post link, the "Replying to" link, the edited-history toggle, and image taps in `MediaGallery`. Middle-clicking a Like button therefore does not also open the post; middle-clicking a hashtag/mention/URL still opens that link, not the post — `ContentLinks` already stopped both `click` and `auxclick`.
...
## Why
Middle-click on a post is expected to behave like a browser link (open in new tab). The previous implementation only added a `window.open` in the click handler but did not suppress the browser's native middle-button autoscroll, which on a tall scrollable feed steals the gesture. The post page, being a shorter non-scrolling surface, masked the bug — users reported it "only works after you've opened the post". For keyboard users, `Tab` highlighted the card but `Enter` did nothing — violates `role="link"` (WCAG: `Enter` must activate a link; `Space` scrolls) and leaves no keyboard path to open the post. Separately, clicking the header `@username` pushed two history entries because `UserHoverCard`’s plain `<a>` bubbled to the Card, producing the `profile → post → feed` back sequence you just reported.

## Files touched
- `web/src/components/FeedPost.tsx` — split `handlePostClick` / `handleAuxClick` + `handleMouseDown` (autoscroll suppression + `noopener` + Ctrl/Meta), added `handleKeyDown` (`Enter` → `navigate`), `onKeyDown={handleKeyDown}` + `onAuxClick={handleAuxClick}` + `onMouseDown={handleMouseDown}` + `focus-visible` ring on the Card, and `onAuxClick`/`onMouseDown` stops on all inner controls that already stopped `click`; added `closest("a,button")` guard in click/auxClick as defense-in-depth.
- `web/src/components/UserHoverCard.tsx:49` — trigger changed from `<a href={profileUrl}>` to `<Link to={profileUrl} onClick/auxClick/mouseDown stopPropagation>` so header avatar/name clicks don’t bubble to the Card.
...
- Playwright (host Chrome) against the isolated preview (`gaggle-fix-feed-middle-click`, Frontend http://localhost:5385):
  ...
  - Keyboard: `Tab` → card focused (`role="link"`), `Enter` → navigates to `/post/:id`; `Space` on card → stays on feed (scroll only); `Enter` on focused Like button → toggles like without navigation; `Ctrl+Enter` on card → no new tab (per spec).
  - Profile back-nav: click header `@username`/`avatar` (UserHoverCard) on feed → `http://localhost:5385/profile/alice`, `goBack()` → `http://localhost:5385/` (feed) directly, not via `/post/:id`; second back → login. Previously it was `profile → post → feed`.
...
## What was changed and why
- **Login badge (`web/src/pages/login-lab/variants/StepFlow.tsx:42-43`):** replaced the `G` lettermark with `gaggle-goose.png` inside the same `rounded-2xl bg-primary overflow-hidden` square; bumped to `h-[4.5rem] w-[4.5rem]` (1.5×, ~72px) and `object-cover object-bottom` so the goose's flat neck sits on the badge's bottom edge instead of floating.
- **Sidebar logo (`web/src/layout/SocialMediaLayout.tsx:140-141`):** same system — `rounded-xl bg-primary overflow-hidden` `h-10 w-10` with `object-cover object-bottom` (was bare `rounded-full` image).
- **Asset (`web/public/gaggle-goose.png`):** rebuilt from max-quality `grok_cutout.png` (4096) → `trim + resize x480 + gravity south + extent 512×512`, so the opaque flat bottom coincides with the container clip; sharp source for favicon/app icons. Favicon left as-is (transparent goose, no background) per request.

## Verification
- Preview `gaggle-login-goose-logo` healthy: Frontend http://localhost:5215, API http://localhost:2115. Hard-refresh recommended (cached 80×80 vs new 512).
